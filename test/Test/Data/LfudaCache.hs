...
testLFUDA = do
  ...
  let keys = getKeys finalCache
  assertEqual "Keys length should match cache length" len (length keys)
  ...
  forM_ [100..333 :: Int] $ \i -> do
    let result = get i finalCache
    assertBool ("Key " ++ show i ++ " should not be in cache") (isNothing result)
  
  -- Set a new value and check it
  let (evicted, cacheWithNewVal) = set (256 :: Int) (256 :: Int) finalCache
  
  let result = get (256 :: Int) cacheWithNewVal
  case result of
  ...
  let keysAfterUpdate = getKeys updatedCache
  ...
  let resultAfterPurge = get (200 :: Int) purgedCache
...
testLFUDAContains = do
  ...
  let keys1 = getKeys finalCache1
  assertEqual "Key 1 should be most frequently used" (1 :: Int) (head keys1)
  
  -- Contains should not bump hits for key 2
  let finalCache2 = foldl' (\c _ -> c) finalCache1 (replicate 20 (contains (2 :: Int) finalCache1))
  
  let keys2 = getKeys finalCache2
  assertEqual "Key 1 should still be most frequently used" (1 :: Int) (head keys2)
...
testLFUDAPeekOrSet = do
  ...
  -- Bump hits for key 3
  let cache3' = case get (3 :: Int) cache3 of
        Just (_, c) -> c
        Nothing -> cache3
...
testLFUDAPeek = do
  ...
  let cache2' = case get (2 :: Int) cache2 of
        Just (_, c) -> c
        Nothing -> cache2
...
testLFUDAAge = do
  ...
  let cache1' = case get (1 :: Int) cache1 of
        Just (_, c) -> c
        Nothing -> cache1
  ...
  let age = getAge cache2
  assertEqual "Cache age should be 1" 1 age
